...
Using the browser App, the user will make a _POST {collection_id}/{dataset_id}/upload/link_ request to upload the file
...
by providing a hash of the data. We can compare this hash with the hash of downloaded file. If the hash does not match 
we can reattempt the download.
...
Before uploading we will need to calculate
the expected hash. We can use the [Content-MD5 header](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html#API_PutObject_RequestSyntax) 
...
Using the access token and the AWS JS SDK we will upload the file directly to S3. We will calculate the hash of each chunk
locally and add the hash to [Content-MD5 header](https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html#API_PutObject_RequestSyntax) 
in the Put Object request. This will verify the integrity of each chunk during the upload. If the hash does not match
