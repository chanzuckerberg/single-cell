...
their CSP that is used to upload directly to S3.
...
1. The publisher must also agree to portal policies as outlined in
   [Publishers must complete attestation before submitting private datasets to the portal](https://app.zenhub.com/workspaces/single-cell-5e2a191dad828d52cc78b028/issues/chanzuckerberg/single-cell/22).
...
performing quick validation of the filing being uploaded
  ...
  "Statement": [
    {
      ...
      "Resource": ["arn:aws:s3:::upload-bucket/{data set_uuid}/{asset_uuid}"]
    }
  ]
}
```
...
##### Request: POST collection/{collection_id}/{dataset_id}/upload/file

- File size - the size of the file to be uploaded
- collection_id - identifies the collection
- attestation - A boolean value indicating if the file is for attestation
- **Optional** asset_uuid - identifies the name of the new file created.

##### Response: POST collection/{collection_id}/{dataset_id}/upload/file
...
Using the browser App, make a _POST {collection_id}/{dataset_id}/upload/link_ request to upload the file
