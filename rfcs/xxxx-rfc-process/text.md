...
Previously the "RFC" (a.k.a. "design doc review") process was executed on Google Docs. There were two problems: first is that updates were quite tricky to make (do you update the doc directly, do you have a changelog, which version is latest) and the second was the designs decisions were easily accessibly by our external community. In order to alleviate these concerns, we have moved to Github to keep track of our RFCs moving forward.
...
2. Create a new directory under the `rfcs` directory named `XXXX-name-of-rfc`.
...
1. Create a new branch of the `cellxgene-rfcs` repo.
2. Make your changes. If there are new approvers, add them to the list of approvers at the top of the RFC.
